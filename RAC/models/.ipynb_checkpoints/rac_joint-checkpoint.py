...
#                      test_indices=None):
#     """
#     Evaluate the model on test data, using the combined index (train+val).
#     Also return a DataFrame with [Index, Predicted, Actual].
#     """
#     model.eval()
#     with torch.no_grad():
#         # Prepare test features (the same weighted neighbor approach)
#         batch_features = prepare_batch_features(
#             test_embeddings, 
#             combined_embeddings, 
#             combined_labels, 
...
def rac_joint_main(folder, k):
    ...
    # class_weights = [total_samples / label_counts[i] for i in range(output_dim)]
    ...
    # Update class weights for combined data
    # all_combined_labels = []
    # for emb, lbl in combined_loader:
    #     all_combined_labels.extend(lbl.numpy())

    # label_counts = Counter(all_combined_labels)
    # total_samples = sum(label_counts.values())
    # class_weights = [total_samples / label_counts[i] for i in range(output_dim)]
    # class_weights_tensor = torch.tensor(class_weights, dtype=torch.float32).to(device)
    ...
    # Evaluate on Test Data
    # ----------------------------
    # test_f1, test_auroc_macro, test_auroc_micro, conf_matrix = evaluate_on_test(
    #     model,
    #     test_embeddings, 
    #     test_labels,
    #     combined_embeddings,
